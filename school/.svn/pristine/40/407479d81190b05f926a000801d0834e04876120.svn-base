  ...
  <div id="courseScoreInputToolbar" style="padding: 5px; height: auto">
   <input type="hidden" id="courseId" value="${courseId }">
    <div><input name="scoreType" type="radio" class="easyui-validatebox"
			...
			<span style="margin-left:80px" >考试日期：</span>	
...
$(function(){
	...
	$("#resetCourseScore").click(function(){
	 ...
	 $('#tt').datagrid({
			...
			singleSelect:true,
			idField:'id',
			url:'${contextPath}/edu/editCourseScore.do?courseId=' + $("#courseId").val(),
	...
	$("#startInput").click(function(){
		var examDate = $("#examDate").textbox('getValue');
		 ...
		 $('#tt').datagrid({
				...
				singleSelect:true,
				idField:'id',
				url:'${contextPath}/edu/editCourseScore.do',
				queryParams:param,
				onClickRow: onClickRow,
				columns:columns
			});
...
function submitCourseScore() {
	...
	var param = {"numberboxs":numberboxs,"examScore":examScores,"totalScore":totalScores,
			"examNature":examNatures,"studentId":studentIds,"scoreType":scoreType,"teachCourseId":$("#courseId").val()}
